...
export default function HodGroupsPage() {
  ...
  // Unique guides and divisions for filter dropdowns
  const guideNames = [...new Set(groups.map(g => g.guide?.name).filter(Boolean))] as string[]
  const divisions = [...new Set(groups.map(g => g.division).filter(Boolean))]
  ...
  const filtered = groups.filter(g => {
    ...
    if (filterGuide && g.guide?.name !== filterGuide) return false
    if (filterStatus) {
      const st = g.project?.status ?? 'NONE'
      if (filterStatus === 'NONE' && g.project) return false
  ...
  useEffect(() => { setPage(1) }, [search, filterYear, filterDivision, filterGuide, filterStatus])

  const handleExport = async () => {
    try {
      const res = await api.get('/export/groups')
      downloadAsExcel(res.data, 'department_groups_export')
    } catch { /* */ }
  }
  ...
  const openCreate = async () => {
    setShowCreate(true)
    setCreateForm({ name: '', year: 'TY', division: '', academicYear: '', semester: 1, guideId: '' })
    setCreateStudentIds([])
    ...
    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <select value={filterDivision} onChange={e => setFilterDivision(e.target.value)} className={`${inputCls} w-auto min-w-[120px]`}>
        ...
        <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)} className={`${inputCls} w-auto min-w-[130px]`}>
                  ...
                  <td className="py-3 px-4">
                    {g.project ? (
                      <span className={`text-[10px] px-2 py-0.5 rounded-full border ${
                        ...
                        g.project.status === 'APPROVED' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' :
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }}
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="block text-xs text-[#7A8BAF] mb-1.5">Students (min 3, max 5 — first selected = leader)</label>
                  ...
                  {createStudentIds.length > 0 && (
                    <p className={`text-xs mb-2 ${createStudentIds.length < 3 ? 'text-red-400' : createStudentIds.length > 5 ? 'text-red-400' : 'text-amber-400'}`}>
                      {createStudentIds.length} selected {createStudentIds.length < 3 ? '(need at least 3)' : createStudentIds.length > 5 ? '(max 5)' : ''}
                    </p>
                  )}
                  <div className="max-h-40 overflow-y-auto space-y-1">
                    {createStudents
                      ...
                      .map(s => (
                        <button key={s.id} onClick={() => setCreateStudentIds(prev =>
                          prev.includes(s.id) ? prev.filter(x => x !== s.id) : prev.length >= 5 ? prev : [...prev, s.id]
              ...
              <button onClick={handleCreate} disabled={!createForm.name || !createForm.division || createStudentIds.length < 3 || actionLoading}
